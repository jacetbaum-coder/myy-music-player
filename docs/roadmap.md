...
* Hardened guest crate migration so auth success captures an explicit guest crate snapshot before the identity switch, persists a pending migration backup for the signed-in account, retries that merge on identity refresh, and only clears the guest crate after the account crate save plus cloud sync succeeds.
