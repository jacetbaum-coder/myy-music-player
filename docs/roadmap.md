...
* Added a dedicated account download roadmap at `/Users/jtannenbaum/myy-music-player/docs/account-download-roadmap.md` and completed Step 1 by turning the crate import tile into an explicit `Download Your Music` entry that stays visible to guests, shows account-required copy, and still routes every import attempt through account auth.
* Completed Step 2 of the account download roadmap by turning the import field into a unified text-or-link entry point, adding frontend search result selection and YouTube preview UI before download, and preserving direct Spotify link fallback behavior for now.
* Completed Step 3 of the account download roadmap by implementing the local downloader `/search` and `/preview` endpoints, installing the missing downloader packages into the repo venv, fixing helper startup to prefer the repo venv automatically, and verifying live normalized search and preview responses.
* Expanded Step 4 of the account download roadmap by preserving selected source metadata through download, showing matched preview metadata again in review, auto-opening review after resolved downloads, and prefilling missing review tags from the chosen preview before upload.
* Expanded Step 5 of the account download roadmap by scoping uploader and shared-library copy keys into `users/<userId>/...`, switching the post-upload Done action toward the authenticated `/user/songs` route, making the main library bootstrap use a separate personal cache plus the authenticated personal-library refresh path when personal mode is active, exposing a shared mode-aware library refresh helper so personal mode is not tied to import-specific code paths, merging shared plus personal albums during personal-mode library refresh so uploaded tracks no longer replace the shared catalog, and adding clearer auth-expiry feedback plus background personal-library refresh after successful copy-to-library actions.
...
* Run a browser-level signed-in verification pass for Step 4 and Step 5 of `/Users/jtannenbaum/myy-music-player/docs/account-download-roadmap.md`: search result selection into review, authenticated upload into `users/<userId>/...`, and personal-library refresh staying in personal mode.
* After that verification pass, mark Step 4 done if the review handoff is solid, then either finish Step 5 or document any remaining ownership bugs before starting Step 6 merged-library integration.
...
Last completed step: Phase 1: Lock product rules. Next actual roadmap step: Phase 1A: Clean up identity boundaries (updated). Prerequisite sub-step: run the remaining signed-in verification for guest playlist migration, guest-mode gating, merged library behavior, and history or now-playing write flows against the already-deployed session-backed routes. Parallel implementation note: Step 3 of `/Users/jtannenbaum/myy-music-player/docs/account-download-roadmap.md` is complete, and Step 4 plus Step 5 are in progress pending signed-in browser verification.