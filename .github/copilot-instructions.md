...
8. **Chrome iOS safe-area / home indicator strip** — `env(safe-area-inset-bottom)` returns `0` in Chrome iOS (including Add-to-Home-Screen PWA) even though the property is supported, so any fallback value like `env(..., 34px)` never fires. Additionally, `position: fixed; bottom: 0` in Chrome iOS lands at the *safe viewport boundary* (above the home indicator), not the physical screen bottom. Fix: a JS probe at page load checks `env()` and if it's 0, uses `iPhone` UA + `screen.height/width > 736` (Face ID phones) to set `--safe-area-bottom-actual: 34px` and `--nav-translate-y: 34px` on `<html>`. The nav CSS uses `height: calc(var(--nav-height) + var(--safe-area-bottom-actual, ...))` to cover the inset zone, and `transform: translateY(var(--nav-translate-y, 0px))` to push the nav past the safe-area boundary. Safari standalone sets `env()` correctly and takes a separate early-return path — no translate needed there. **Do not gate this on `display-mode: standalone`** — Chrome iOS Add-to-Home-Screen does not report standalone mode.
